...
LOG_MODULE_REGISTER(sim7600_cellular, CONFIG_MODEM_LOG_LEVEL);

#include <string.h>
#include <stdlib.h>

#undef MODEM_CELLULAR_DEFINE_INSTANCE
#define MODEM_CELLULAR_DEFINE_INSTANCE(inst, power_ms, reset_ms, startup_ms, shutdown_ms, start,   \
	...
	static const struct modem_cellular_config MODEM_CELLULAR_INST_NAME(config, inst) = {       \
		...
		.user_pipes_size = ARRAY_SIZE(MODEM_CELLULAR_GET_USER_PIPES(inst)),                \
                                                                                                   ...
                                                                                                   \
	PM_DEVICE_DT_INST_DEFINE(inst, sim7600_cellular_pm_action);                                \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, sim7600_cellular_init, PM_DEVICE_DT_INST_GET(inst),            \
			      &MODEM_CELLULAR_INST_NAME(data, inst),                               \
			      &MODEM_CELLULAR_INST_NAME(config, inst), POST_KERNEL,                \
			      CONFIG_MODEM_CELLULAR_INIT_PRIORITY, &sim7600_cellular_api);
...
static void sim7600_cellular_chat_callback_handler(struct modem_chat *chat,
						   ...
						   void *user_data)
{
	...
	if (result == MODEM_CHAT_SCRIPT_RESULT_SUCCESS) {
		...
		if (chat != NULL && chat->script != NULL &&
		    chat->script_chat_it < chat->script->script_chats_size) {
			script_chat = &chat->script->script_chats[chat->script_chat_it];
			LOG_WRN("SIM7600 script '%s' failed at step %u on request '%.*s'",
				chat->script->name, chat->script_chat_it, script_chat->request_size,
				script_chat->request != NULL ? (const char *)script_chat->request : "");
		}
...
MODEM_CHAT_MATCHES_DEFINE(__maybe_unused dial_abort_matches,
	...
	DT_HAS_COMPAT_STATUS_OKAY(simcom_sim7600)
							 ...
							 enum modem_cellular_event evt)
{
	...
	case MODEM_CELLULAR_EVENT_SCRIPT_FAILED:
		LOG_WRN("SIM7600 init script failed");
		if (modem_cellular_gpio_is_enabled(&config->reset_gpio) &&
		    config->reset_on_recovery) {
			LOG_WRN("SIM7600 recovering with reset pulse");
			modem_cellular_enter_state(data, MODEM_CELLULAR_STATE_RESET_PULSE);
			break;
		}

		LOG_WRN("SIM7600 init failure falling back to idle without power repulse");
		modem_cellular_enter_state(data, MODEM_CELLULAR_STATE_IDLE);
		break;
							...
							enum modem_cellular_event evt)
{
	...
	case MODEM_CELLULAR_EVENT_SCRIPT_FAILED:
		...
		LOG_WRN("SIM7600 APN script failed");
		if (modem_cellular_is_script_retry_exceeded(data)) {
			LOG_WRN("SIM7600 APN retries exhausted, requesting suspend");
							 ...
							 enum modem_cellular_event evt)
{
	...
	case MODEM_CELLULAR_EVENT_SCRIPT_FAILED:
		modem_cellular_script_failed(data);
		LOG_WRN("SIM7600 dial script failed");
		if (modem_cellular_is_script_retry_exceeded(data)) {
			LOG_WRN("SIM7600 dial retries exhausted, requesting suspend");
			modem_cellular_delegate_event(data, MODEM_CELLULAR_EVENT_SUSPEND);
		} else {
			modem_cellular_start_timer(data, MODEM_CELLULAR_PERIODIC_SCRIPT_TIMEOUT);
		}
							...
							enum modem_cellular_event evt)
{
	...
	switch (evt) {
	case MODEM_CELLULAR_EVENT_CMUX_DISCONNECTED:
		LOG_WRN("SIM7600 entering init power-off after CMUX disconnect");
		modem_cellular_stop_timer(data);
		...
		break;
	case MODEM_CELLULAR_EVENT_TIMEOUT:
		LOG_WRN("SIM7600 init power-off timeout, shutdown script %savailable",
			(config->shutdown_chat_script != NULL && data->cmd_pipe != NULL) ? "" : "not ");
...
static int modem_cellular_on_power_off_pulse_state_enter(struct modem_cellular_data *data)
{
	...
	data->cmd_pipe = NULL;
	LOG_WRN("SIM7600 entering power-off pulse, external power GPIO remains app-controlled");
	modem_cellular_start_timer(data, K_MSEC(config->power_pulse_duration_ms));
	return 0;
}
...
{
	LOG_WRN("SIM7600 leaving power-off pulse, external power GPIO unchanged");
	modem_cellular_stop_timer(data);
	return 0;
}
...
MODEM_CHAT_SCRIPT_DEFINE(get_signal_csq_chat_script, get_signal_csq_chat_script_cmds,
			 abort_matches, sim7600_cellular_chat_callback_handler, 2);
...
DEVICE_API(cellular, sim7600_cellular_api) = {
...
int sim7600_cellular_pm_action(const struct device *dev, enum pm_device_action action)
{
	struct modem_cellular_data *data = (struct modem_cellular_data *)dev->data;
	int ret;
...
static void modem_cellular_init_apn(struct modem_cellular_data *data)
{
	...
	modem_chat_script_set_callback(&data->apn_script,
				       sim7600_cellular_chat_callback_handler);
}

static int sim7600_cellular_init(const struct device *dev)
{
	struct modem_cellular_data *data = (struct modem_cellular_data *)dev->data;
	...
	if (modem_cellular_gpio_is_enabled(&config->power_gpio)) {
		LOG_DBG("SIM7600 power GPIO present but app-controlled");
	}
...
MODEM_CHAT_SCRIPT_DEFINE(quectel_bg9x_init_chat_script, quectel_bg9x_init_chat_script_cmds,
			 abort_matches, sim7600_cellular_chat_callback_handler, 10);
...
MODEM_CHAT_SCRIPT_DEFINE(quectel_bg9x_dial_chat_script, quectel_bg9x_dial_chat_script_cmds,
			 dial_abort_matches, sim7600_cellular_chat_callback_handler, 10);
...
MODEM_CHAT_SCRIPT_DEFINE(quectel_bg9x_periodic_chat_script,
			 ...
			 sim7600_cellular_chat_callback_handler, 4);
...
MODEM_CHAT_SCRIPT_DEFINE(quectel_bg9x_shutdown_chat_script,
			 ...
			 sim7600_cellular_chat_callback_handler, 1);
#endif
...
MODEM_CHAT_SCRIPT_DEFINE(quectel_eg800q_init_chat_script, quectel_eg800q_init_chat_script_cmds,
			 abort_matches, sim7600_cellular_chat_callback_handler, 30);
...
#if DT_HAS_COMPAT_STATUS_OKAY(simcom_sim7600)
MODEM_CHAT_SCRIPT_CMDS_DEFINE(simcom_sim7600_init_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT", ok_match),
...
MODEM_CHAT_SCRIPT_DEFINE(simcom_sim7600_init_chat_script, simcom_sim7600_init_chat_script_cmds,
			 abort_matches, sim7600_cellular_chat_callback_handler, 10);

MODEM_CHAT_SCRIPT_CMDS_DEFINE(simcom_sim7600_dial_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP_MULT("AT+CGACT=0,1", allow_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CFUN=1", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("ATD*99***1#", connect_match),);

MODEM_CHAT_SCRIPT_DEFINE(simcom_sim7600_dial_chat_script, simcom_sim7600_dial_chat_script_cmds,
			 dial_abort_matches, sim7600_cellular_chat_callback_handler, 10);

MODEM_CHAT_SCRIPT_CMDS_DEFINE(simcom_sim7600_periodic_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CREG?", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CEREG?", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CGREG?", ok_match));

MODEM_CHAT_SCRIPT_DEFINE(simcom_sim7600_periodic_chat_script,
			 simcom_sim7600_periodic_chat_script_cmds, abort_matches,
			 sim7600_cellular_chat_callback_handler, 4);

MODEM_CHAT_SCRIPT_CMDS_DEFINE(simcom_sim7600_shutdown_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CPOF", ok_match));

MODEM_CHAT_SCRIPT_DEFINE(simcom_sim7600_shutdown_chat_script,
			 simcom_sim7600_shutdown_chat_script_cmds, abort_matches,
			 sim7600_cellular_chat_callback_handler, 15);
...
MODEM_CHAT_SCRIPT_DEFINE(u_blox_lara_r6_set_baudrate_chat_script,
			 ...
			 abort_matches, sim7600_cellular_chat_callback_handler, 1);
...
MODEM_CHAT_SCRIPT_DEFINE(nordic_nrf91_slm_shutdown_chat_script,
			 ...
			 sim7600_cellular_chat_callback_handler, 2);
...
MODEM_CHAT_SCRIPT_DEFINE(sqn_gm02s_dial_chat_script, sqn_gm02s_dial_chat_script_cmds,
			 dial_abort_matches, sim7600_cellular_chat_callback_handler, 15);
...
#define MODEM_CELLULAR_DEVICE_SIMCOM_SIM7600(inst)                                                 \
	MODEM_DT_INST_PPP_DEFINE(inst, MODEM_CELLULAR_INST_NAME(ppp, inst), NULL, 98, 1500, 64);   \
                                                                                                   \
	static struct modem_cellular_data MODEM_CELLULAR_INST_NAME(data, inst) = {                 \
		.chat_delimiter = "\r",                                                            \
	...
	MODEM_CELLULAR_DEFINE_INSTANCE(inst, 500, 100, 20000, 5000, false,                         \
				       NULL,                                                       \
				       &simcom_sim7600_init_chat_script,                           \
				       &simcom_sim7600_dial_chat_script,                           \
				       &simcom_sim7600_periodic_chat_script,                       \
				       &simcom_sim7600_shutdown_chat_script)
...
DT_INST_FOREACH_STATUS_OKAY(MODEM_CELLULAR_DEVICE_SIMCOM_SIM7600)
#undef DT_DRV_COMPAT
